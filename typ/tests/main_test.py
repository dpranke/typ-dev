...
class TestCli(test_case.MainTestCase):
    ...
    def test_skip_via_expectations(self):
        ...
        # output bugs.
        self.assertIn(
            '[1/2] fail_test.FailingTest.test_fail was skipped', out)
    ...
    def test_skip_via_expectations_at_runtime_no_associated_bugs(self):
        ...
        _, out, _, _ = self.check(['-j', '1', '-X', 'expectations.txt'],
                                  files=files, ret=0)
        self.assertIn(
            '[2/2] skip_test.SkipTestSetTags.test_skip was skipped', out)
    ...
    def test_timing(self):
        self.check(['-wt'], files=PASS_TEST_FILES, ret=0, err='',
    ...
    def test_skip_test_with_expectations_file_skip_expectation(self):
        ...
        _, out, _, files = self.check(['--write-full-results-to',
                                      ...
                                      files=files, ret=0, err='')
        self.assertIn(
                '[1/1] fail_test.FailingTest.test_fail was skipped\n',
                out)
        self.assertIn('0 tests passed, 1 skipped, 0 failures.\n', out)
        results = json.loads(files['full_results.json'])
    ...
    def test_skip_test_with_expectations_file_skip_with_cmd_args(self):
        ...
        _, out, _, files = self.check(['--write-full-results-to',
                                      ...
                                      files=files, ret=0, err='')
        self.assertIn(
                '[1/1] pass_test.PassingTest.test_pass was skipped\n',
                out)
        self.assertIn('0 tests passed, 1 skipped, 0 failures.\n', out)
        results = json.loads(files['full_results.json'])
    ...
    def test_skip_test_no_expectations_file(self):
        ...
        _, out, _, files = self.check(['--write-full-results-to',
                                      ...
                                      files=files, ret=0, err='')
        self.assertIn(
            '[1/1] skip_test.SkipTest.test_skip was skipped\n', out)
        self.assertIn('0 tests passed, 1 skipped, 0 failures.\n', out)
        results = json.loads(files['full_results.json'])
    ...
    def test_verbose_3(self):
        # Note that -v enables -w automatically.
